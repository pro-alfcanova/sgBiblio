...
// Função para buscar o próximo código de leitor
function buscarUltimoLeitor() {
  const sheetLeitor = obterPlanilhaLeitores();
  const lastRow = sheetLeitor.getLastRow();
  
  if (lastRow <= 1) return 1;

  const lastCodigo = sheetLeitor.getRange(lastRow, 1).getValue();
  return lastCodigo + 1;
}
...
function incluirLeitor(leitorIncluir) {
  ...
  for (let i = 0; i < leitoresExistentes.length; i++) {
    if (String(leitoresExistentes[i][0]).trim() === String(codigo_do_leitor).trim()) {
      return { success: false, message: "Já existe um leitor cadastrado com este código!" };
    }
  }
  ...
  return { success: true, message: "Cadastro do leitor incluído com sucesso!" };
}
...
function buscarLeitor(codigo_leitor) {
  ...
  if (leitor && !emprestimo) {
    ...
    return `Leitor possui um empréstimo desde ${emprestimo[5]} com data de devolução para ${emprestimo[6]}`;
  }
  return `Leitor não encontrado ou já excluído!`;
}
...
function alterarLeitor(leitorAlterar) {
  ...
  if (linhaAlvo > 0) {
    sheetLeitor.getRange(linhaAlvo, 1, 1, 5).setValues([[
      codigo_do_leitor, nome_do_leitor, funcao_do_leitor, local_do_leitor, contato_do_leitor
    ]]);
    return { success: true, message: "Cadastro do leitor atualizado com sucesso!" };
  } 
  return { success: false, message: "Leitor não encontrado ou já excluído." };
}
...
function excluirLeitor(leitorExcluir) {
  ...
  if (linhaAlvo > 0) {
    ...
    sheetLeitor.getRange(linhaAlvo, 1, 1, 7).setValues([[
      codigo_do_leitor, nome_do_leitor, funcao_do_leitor, local_do_leitor, contato_do_leitor, data_de_inclusao, data_de_exclusao
    ]]);
    return { success: true, message: "Cadastro do leitor excluído com sucesso!" };
  }
  return { success: false, message: "Leitor não encontrado ou já excluído." };
}
